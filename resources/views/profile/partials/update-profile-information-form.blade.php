@section('js')
<script src="{{url('data.js')}}"></script>
<script src="{{asset('site/custom/address_custom_profile.js')}}"></script>
@endsection
@section('css')
<style>
    div input {
    ...
    <form method="post" action="{{ route('profile.update') }}" class="mt-6 space-y-6" enctype="multipart/form-data">
        ...
        <div>
            ...
            <select id="gender" name="gender" class="mt-1 block w-full border border-1">
                ...
                @case(Auth::user()->gender == "female")
                <option value="male" selected>Male</option>
                <option value="female" selected>Female</option>
                <option value="other">Other</option>
                @break
                @case(Auth::user()->gender == "other")
        ...
        <div>
            <x-input-label for="city" :value="__('City/Province')" />
            <select id="city" class="mt-1 block w-full">
                <option value="{{Auth::user()->city}}">{{Auth::user()->city}}</option>
            </select>
            <input type="text" name="city" id="city_post" value="">

        </div>
        <div>
        </div>
        <div>
            <x-input-label for="distrist" :value="__('Distrist')" />
            <select id="district" class="mt-1 block w-full">
                <option value=" {{Auth::user()->district}}">{{Auth::user()->district}}</option>
            </select>
            <input type="text" name="district" id="district_post" value="">
        ...
        <div>
            ...
            <select id="ward" class="mt-1 block w-full">
                <option value=" {{Auth::user()->ward}}">{{Auth::user()->ward}}</option>
            </select>
            <input type="text" name="ward" id="ward_post" value="">
